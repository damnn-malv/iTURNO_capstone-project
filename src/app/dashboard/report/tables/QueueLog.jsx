...
import { matchesQueueRow, matchesRoamingRow, useDebouncedSearchAll } from "../reportHook";
const QUEUE_COLUMNS = ["Timestamp", "Ticket ID", "Action", "Driver", "Vehicle", "Route", "User"];
...
export default function QueueLog({
  ...
  const data = isQueue ? queueData : roamingData;
  const meta = isQueue ? queueMeta : roamingMeta;
  const fetchPage = isQueue ? onQueueFetchPage : onRoamingFetchPage;
  const fetchAll = isQueue ? onQueueFetchAll : onRoamingFetchAll;

  const matches = isQueue ? matchesQueueRow : matchesRoamingRow;
  ...
  const renderQueueRow = (l, idx, { rowClass, cellClass }) => (
  ...
  const columns = isQueue ? QUEUE_COLUMNS : ROAMING_COLUMNS;
  const rowRenderer = isQueue ? renderQueueRow : renderRoamingRow;
  ...
  return (
    <div className="rpt-card rpt-section">
      <div className="rpt-card-header">
        <div className="rpt-card-header-left">
          <div className="rpt-tab-group">
            <button
              className={`rpt-tab ${isQueue ? "rpt-tab--active" : ""}`}
        ...
        <div className="rpt-card-header-actions">
          ...
          <button
            ...
            onClick={() => (isQueue ? onExportQueueCSV(search) : onExportRoamingCSV(search))}
          >
          ...
          <button
            ...
            title={search ? "Export only rows matching your search" : "Export all records in range"}
            onClick={() => (isQueue ? onExportQueuePDF(search) : onExportRoamingPDF(search))}
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
      ...
      {showModal && (
        <ReportTableModal
          title={isQueue ? "Terminal Queue Log" : "Roaming Logs"}
          subtitle={isQueue ? "Full ticket activity history" : "Full roaming vehicle activity history"}
          count={modalTotal}
          onClose={closeModal}
          searchValue={modalSearch}
          onSearchChange={setModalSearch}
          searchPlaceholder={isQueue ? "Search logs…" : "Search roaming…"}
